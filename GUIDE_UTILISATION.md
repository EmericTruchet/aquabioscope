...
Aucune installation requise : décompresse le dossier `DivePhoto` reçu et double-clique sur `DivePhoto.exe` à l'intérieur. Le dossier entier (`DivePhoto.exe` + le sous-dossier `_internal`) doit rester ensemble — ne déplace pas l'exécutable seul.
...
1. **Nouvelle session** : indique le dossier contenant tes photos brutes (JPG ou RAW), un dossier de sortie (créé automatiquement s'il n'existe pas), le lieu et la date de la plongée, la zone (Méditerranée / Atlantique / les deux — utilisée pour filtrer la liste d'espèces), et ton nom pour le crédit photo.
2. **Retouche** : pour chaque photo, choisis l'une des 3 propositions colorimétriques (ou l'original) en cliquant sur une vignette ou avec les touches `1`-`4`. Valide avec Entrée ou en cliquant sur l'aperçu. `Suppr` écarte une photo ratée.
3. **Espèces** : recherche et ajoute les espèces visibles sur la photo (nom commun ou scientifique, avec leur embranchement). Une espèce absente de la base peut être ajoutée librement.
